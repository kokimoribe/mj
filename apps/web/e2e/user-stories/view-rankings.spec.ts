...
test.describe("User Story: Viewing Current Rankings", () => {
  test("As Joseph, I want to check if I'm still in first place", async ({
    page,
  }) => {
    ...
    const josephCard = page
      .locator('[role="button"]', { hasText: "Joseph" })
      .first();
    await expect(josephCard).toBeVisible();

    // He sees his rating (use the specific class for the main rating)
    await expect(
      josephCard.locator(".text-2xl.font-bold").first()
    ).toBeVisible();
  ...
  test("As Koki, I want to see if my rating improved after last night's games", async ({
    page,
  }) => {
    ...
    const kokiCard = page
      .locator('[role="button"]', { hasText: /koki/i })
      .first();
  ...
  test("As a new player, I want to see where I rank among all players", async ({
    ...
    const jackieCard = page
      .locator('[role="button"]', { hasText: /jackie/i })
      .first();
    ...
    if (cardCount > 0) {
      ...
      await expect(
        jackieCard.locator(".text-2xl.font-bold").first()
      ).toBeVisible();
