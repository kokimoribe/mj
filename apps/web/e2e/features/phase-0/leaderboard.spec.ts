...
test.describe('Phase 0: Leaderboard Features', () => {
  ...
  test('expands player card to show detailed statistics', async ({ page }) => {
    ...
    await expect(page.getByText('Season Total')).toBeVisible();
    await expect(page.getByText('Recent Games')).toBeVisible();
    await expect(page.getByText('View Full Profile')).toBeVisible();
  });

  test('navigates to player profile when clicking profile button', async ({ page }) => {
    await navigateTo(page, '/');
  ...
  test('shows loading state while fetching data', async ({ page }) => {
    ...
    const skeleton = page.locator(`[data-testid="${TEST_IDS.LOADING_SKELETON}"]`).first();
