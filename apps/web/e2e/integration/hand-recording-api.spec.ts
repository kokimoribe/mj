...
test.describe("Hand Recording API - Real-time Updates", () => {
  test("WebSocket subscription for live hand updates", async ({ page }) => {
    await page.goto("/games/active");

    // Set up WebSocket listener
    await page.evaluate(() => {
      window.handUpdates = [];

      // Subscribe to hand updates (Supabase real-time)
      const channel = window.supabase
        ...
        .on(
          ...
          (payload: any) => {
            window.handUpdates.push(payload);
          }
        )
        .subscribe();
    });

    // Record a hand via API
    await page.evaluate(async () => {
      const response = await fetch("/api/games/active-game/hands", {
        method: "POST",
    ...
    await page.waitForFunction(() => window.handUpdates.length > 0, {
      timeout: 5000,
    });

    // Verify update received
    const updates = await page.evaluate(() => window.handUpdates);
    expect(updates).toHaveLength(1);
    expect(updates[0].new).toHaveProperty("outcome_type", "ron");
  });
});
