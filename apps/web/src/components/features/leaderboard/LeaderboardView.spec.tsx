...
describe("PWA Leaderboard Component Tests", () => {
  ...
  describe("Data Model Requirements", () => {
    it("displays all required player fields", () => {
      ...
      const josephCard = screen
        .getByText("Joseph")
        .closest('[data-testid^="player-card-"]') as HTMLElement;
  ...
  describe("Season Summary", () => {
    ...
    it('shows correct "Updated X ago" based on materialized_at timestamp', () => {
      ...
      expect(
        screen.getByText(/Updated.*[0-9]+[hm]?\s*ago/)
      ).toBeInTheDocument();
    });
  });
  ...
  describe("Expandable Cards", () => {
    it("shows additional stats when expanded", async () => {
      ...
      await waitFor(() => {
        ...
        // Win Rate is removed from the UI per specifications
  ...
  describe("Performance Requirements", () => {
    it("renders without layout shift", () => {
      ...
      cards.forEach(card => {
        // Check that the card has min-h-[80px] class which sets minHeight
  ...
  describe("Accessibility", () => {
    it("has proper ARIA labels for screen readers", () => {
      ...
      // Get all player cards - they have role="button" and specific test IDs
      const cards = screen
        .getAllByRole("button")
        .filter(button =>
          button.getAttribute("data-testid")?.startsWith("player-card-")
        );
      ...
      cards.forEach(card => {
        ...
        // Aria labels should contain player name and rating info
        const ariaLabel = card.getAttribute("aria-label");
        expect(ariaLabel).toMatch(/\w+ - Rank \d+\.\d+ - Click to/);
      });
    });
