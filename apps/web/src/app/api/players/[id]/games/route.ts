...
export async function GET(
  ...
  try {
    ...
    const typedGamesData = gamesData as GameSeatWithGame[] | null;

    if (!typedGamesData || typedGamesData.length === 0) {
      return NextResponse.json([]);
    }

    // Get all game IDs to fetch complete game data
    const gameIds = [...new Set(typedGamesData.map(g => g.game_id))];
    ...
    const { data: completeGames } = (await supabase
      ...
      .in("id", gameIds)) as { data: Game[] | null };

    // Format the games
    const games = [];

    for (const gameData of typedGamesData) {
      ...
      const opponents = completeGame.game_seats
        .filter(seat => seat.player_id !== playerData.id && seat.placement)
        .map(seat => ({
