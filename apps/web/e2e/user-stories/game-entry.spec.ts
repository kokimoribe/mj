...
test.describe('User Story: Recording Game Results', () => {
  test('As the league admin, I want to enter last night\'s game results', async ({ page }) => {
    // Admin navigates to game entry (this might be a different URL or require auth)
    await navigateTo(page, '/games/new');
    
    // They should see a form to enter game results
    await expect(page.getByRole('heading', { name: /Add Game|New Game|Enter Results/i })).toBeVisible();
    
    // They need to select 4 players
    const playerSelects = page.locator('select[name*="player"], input[name*="player"]');
    await expect(playerSelects).toHaveCount(4);
    
    // They enter the scores for each player
    const scoreInputs = page.locator('input[name*="score"], input[type="number"]');
    ...
    await expect(page.getByRole('button', { name: /Submit|Save|Add Game/i })).toBeVisible();
    
    await takeScreenshot(page, 'user-stories/admin-enters-game');
  });

  test('As the league admin, I want to verify scores add up to 100,000', async ({ page }) => {
    await navigateTo(page, '/games/new');
